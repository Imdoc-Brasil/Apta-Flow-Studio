...
const initialSchedules = [
    { id: 'SCH-001', collaborator: 'João da Silva', exam: 'Audiometria', scheduledDate: '2024-08-10', status: 'Agendado' },
    { id: 'SCH-002', collaborator: 'Maria Oliveira', exam: 'Acuidade Visual', scheduledDate: '2024-08-15', status: 'Agendado' },
];

type Schedule = typeof initialSchedules[0];

const mockCollaborators = ['João da Silva', 'Maria Oliveira', 'Carlos Pereira'];
const mockExams = ['Audiometria', 'Acuidade Visual', 'Hemograma Completo', 'Espirometria'];
            ...
            <DialogTrigger asChild>
             ...
             <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Preencha os dados para agendar um novo exame para um colaborador.
                    </DialogDescription>
                </DialogHeader>
                 <form id="add-schedule-form" onSubmit={handleAddSchedule}>
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                         ...
                         <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="exam" className="text-right">Exame</Label>
                            <Select name="exam" required>
                                <SelectTrigger className="col-span-3">
                                    <SelectValue placeholder="Selecione o exame" />
                                </SelectTrigger>
                                <SelectContent>
                                    {mockExams.map(e => <SelectItem key={e} value={e}>{e}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="scheduledDate" className="text-right">Data Agendada</Label>
         ...
         {schedules.length > 0 ? (
                ...
                <TableBody>
                    {schedules.map(schedule => (
                        <TableRow key={schedule.id}>
                            <TableCell className="font-medium">{schedule.collaborator}</TableCell>
                            <TableCell>{schedule.exam}</TableCell>
                            <TableCell>{schedule.scheduledDate}</TableCell>
                            <TableCell>
                                <Badge variant={schedule.status === 'Realizado' ? 'secondary' : 'default'}>
                                    {schedule.status}
                                </Badge>
                            </TableCell>
                        </TableRow>
            ...
            <div className="flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm h-96">
                <div className="flex flex-col items-center gap-1 text-center">
                    ...
                    <Button className="mt-4" onClick={() => setIsDialogOpen(true)}>Agendar Exame</Button>
                </div>
            </div>
         )}
      </CardContent>
