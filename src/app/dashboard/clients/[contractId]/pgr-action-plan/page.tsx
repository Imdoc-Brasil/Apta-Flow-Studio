...
const initialActionPlan = [
    { id: 'PA-001', riskId: 'INV-001', action: 'Implementar enclausuramento acústico na fonte de ruído.', responsible: 'Eng. de Segurança', deadline: '2024-09-30', status: 'Em Andamento' },
    { id: 'PA-002', riskId: 'INV-002', action: 'Realizar treinamento de levantamento de peso para estoquistas.', responsible: 'RH / Téc. Segurança', deadline: '2024-08-15', status: 'Concluída' },
    { id: 'PA-003', riskId: 'INV-003', action: 'Realizar medição de iluminamento e adequar luminárias.', responsible: 'Manutenção', deadline: '2024-08-30', status: 'Não Iniciada' },
];
                 ...
                 <form id="add-action-form" onSubmit={handleAddAction}>
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="riskId" className="text-right">ID do Risco</Label>
                            <Input id="riskId" name="riskId" placeholder="Ex: INV-001" className="col-span-3" required/>
                        </div>
                         <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="action" className="text-right">Ação</Label>
                            <Textarea id="action" name="action" className="col-span-3" required/>
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="responsible" className="text-right">Responsável</Label>
                            <Input id="responsible" name="responsible" className="col-span-3" required/>
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
        ...
        {plan.length > 0 ? (
                    ...
                    {plan.map(item => (
                        <TableRow key={item.id}>
                            <TableCell className="font-medium max-w-sm">
                                ...
                                <p className="text-xs text-muted-foreground">Risco: {item.riskId}</p>
                            </TableCell>
                            <TableCell>{item.responsible}</TableCell>
                            <TableCell>{item.deadline}</TableCell>
                            <TableCell>
                                <Badge variant={item.status === 'Concluída' ? 'secondary' : item.status === 'Em Andamento' ? 'default' : 'outline'}>
                                    {item.status}
                                </Badge>
                            </TableCell>
            ...
            <div className="flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm h-96">
                <div className="flex flex-col items-center gap-1 text-center">
                    ...
                    <Button className="mt-4" onClick={() => setIsDialogOpen(true)}>Adicionar Ação</Button>
                </div>
            </div>
        )}
      </CardContent>
