...
import { Save, FileSignature, Printer, Pencil, AlertCircle, File, Shield, Syringe, Briefcase, FilePlus, ChevronRight } from 'lucide-react'
...
export default function ClinicalEvaluationPage() {
  ...
  const FieldsetGroup = ({ children, title }: { children: React.ReactNode, title: string }) => (
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className='md:col-span-2 space-y-6'>
                <FieldsetGroup title="Seção 01: Dados do Colaborador">
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm'>
                      ...
                      <div className='col-span-full'>
                         <InfoField label="Atividades" value="[Atividades do cargo]" />
                      </div>
                       <div className='col-span-full'>
                 ...
                 <FieldsetGroup title="Seção 02: Contexto do Exame">
                    ...
                    <div className='space-y-2'>
                        ...
                        <Textarea disabled value="[Lista de riscos do PCMSO]" rows={3}/>
                    </div>
                     <div className='space-y-2'>
                        <Label>Exames a Serem Realizados</Label>
                        <Textarea disabled value="[Lista de exames do PCMSO]" rows={3}/>
                    </div>
                </FieldsetGroup>
                     ...
                     <Table>
                        ...
                        <TableBody>
                            <TableRow>
                                ...
                                <TableCell className="text-muted-foreground">[120/80]</TableCell>
                             ...
                             <TableRow>
                                ...
                                <TableCell className="text-muted-foreground">[80 / 1.75]</TableCell>
                                <TableCell><Input placeholder='0' className="w-24"/></TableCell>
                                <TableCell><Input placeholder='0' className="w-24"/></TableCell>
                            </TableRow>
                             <TableRow>
                                <TableCell>IMC</TableCell>
                                <TableCell className="text-muted-foreground">[26.1]</TableCell>
                                <TableCell><Input readOnly placeholder='0.0' className="w-24 bg-muted"/></TableCell>
                                <TableCell><Input readOnly placeholder='Sobrepeso' className="w-24 bg-muted"/></TableCell>
                            </TableRow>
                            <TableRow>
                             ...
                             <TableRow>
                                ...
                                <TableCell className="text-muted-foreground">[92]</TableCell>
                                <TableCell><Input placeholder='0' className="w-24"/></TableCell>
                                <TableCell />
                            </TableRow>
                        </TableBody>
                    ...
                    <div className='space-y-4 rounded-md border p-4'>
                        ...
                        <ExamField label="Fuma?">
                           <RadioGroup defaultValue='nao' className='flex gap-4'>
                                <div className='flex items-center space-x-2'><RadioGroupItem value='sim' id='fuma-sim' /><Label htmlFor='fuma-sim'>Sim</Label></div>
                                <div className='flex items-center space-x-2'><RadioGroupItem value='nao' id='fuma-nao' /><Label htmlFor='fuma-nao'>Não</Label></div>
                           </RadioGroup>
                        </ExamField>
                        <ExamField label="Bebida Alcoólica">
                            <Select><SelectTrigger className="w-[180px]"><SelectValue placeholder="Frequência" /></SelectTrigger><SelectContent><SelectItem value="diaria">Diária</SelectItem><SelectItem value="2-3">2-3x Semana</SelectItem><SelectItem value="fds">Fins de Semana</SelectItem><SelectItem value="eventual">Eventual</SelectItem></SelectContent></Select>
                        </ExamField>
                         <ExamField label="Atividade Física">
                            <Select><SelectTrigger className="w-[180px]"><SelectValue placeholder="Frequência" /></SelectTrigger><SelectContent><SelectItem value="sedentario">Sedentário</SelectItem><SelectItem value="2-3">2-3x Semana</SelectItem><SelectItem value="4-5">4-5x Semana</SelectItem><SelectItem value="+6">+6x Semana</SelectItem></SelectContent></Select>
                        </ExamField>
                        <div className='grid grid-cols-2 gap-4'>
                          <ExamField label="Diabetes?"><Checkbox/></ExamField>
                          <ExamField label="Hipertensão?"><Checkbox/></ExamField>
                        </div>
                        <div className='space-y-2'><Label>Doenças Ortopédicas? Quais?</Label><Input /></div>
                        <div className='space-y-2'><Label>Realiza terapias para fatores de riscos psicossociais? Descrever</Label><Input /></div>
                    </div>
                     {/* Subseção 03 */}
                     <div className='space-y-4 rounded-md border p-4'>
                        <h4 className='font-medium'>Histórico Ocupacional</h4>
                        <div className='space-y-2'><Label>Cargo no emprego anterior:</Label><Input /></div>
                        <div className='space-y-2'><Label>Quais atividades realizava:</Label><Textarea rows={2} /></div>
                         <div className='space-y-2'><Label>Já teve algum Acidente de Trabalho? Descreva. Foi afastado pelo INSS?</Label><Textarea rows={2} /></div>
                          <div className='space-y-2'><Label>Afastamento pelo INSS por doença comum? Descreva:</Label><Textarea rows={2} /></div>
                     </div>
                      {/* Subseção 04 */}
                     <div className='space-y-4 rounded-md border p-4'>
                        <h4 className='font-medium'>Queixas Atuais e Anamnese</h4>
                        <Textarea placeholder='[Campo para IA transcrever a conversa ou para digitação livre]' rows={5}/>
                     </div>
                     {/* Subseção 05 */}
                    <div className='space-y-4 rounded-md border p-4'>
                ...
                <FieldsetGroup title="Seção 06: Achados e Observações">
                    <Textarea disabled value="[Carregar automaticamente os achados alterados]" />
                    <Label>Observações Adicionais</Label>
                    <Textarea placeholder='Adicione observações ou notas adicionais aqui...'/>
                </FieldsetGroup>
                 <FieldsetGroup title="Seção 07: Conclusão">
                    <RadioGroup className="space-y-2">
                        <div className='flex items-center space-x-2'><RadioGroupItem value='normal' id='conc-normal' /><Label htmlFor='conc-normal'>Exame normal</Label></div>
                        <div className='flex items-center space-x-2'><RadioGroupItem value='achado-sem-incapacidade' id='conc-achado-sem' /><Label htmlFor='conc-achado-sem'>Achados alterados, mas que não incapacita para o trabalho</Label></div>
                        <div className='flex items-center space-x-2'><RadioGroupItem value='revisor' id='conc-revisor' /><Label htmlFor='conc-revisor'>Achados alterados, encaminho para avaliação complementar do médico revisor</Label></div>
                        <div className='flex items-center space-x-2'><RadioGroupItem value='parecer' id='conc-parecer' /><Label htmlFor='conc-parecer'>Achados alterados, solicito parecer complementar</Label></div>
                    </RadioGroup>
                </FieldsetGroup>
                <FieldsetGroup title="Seção 08: Ações">
                    <div className='flex flex-wrap gap-2'>
                        <Button variant="outline"><Printer className='mr-2 h-4 w-4' /> Imprimir Recomendações</Button>
                        <Button><FileSignature className='mr-2 h-4 w-4' /> Finalizar Atendimento</Button>
                        <Button variant="secondary"><Save className='mr-2 h-4 w-4' /> Salvar, sem finalizar</Button>
                    </div>
                </FieldsetGroup>
            </div>
                ...
                <Card>
                    <CardHeader>
                     ...
                     <CardContent className="space-y-4 text-sm">
                        <div className="flex justify-between items-center"><span>Avaliação Clínica</span> <Badge variant="secondary">Concluído</Badge></div>
                        <div className="flex justify-between items-center"><span>Audiometria</span> <Badge variant="outline">Pendente</Badge></div>
                        <div className="flex justify-between items-center"><span>Acuidade Visual</span> <Badge variant="outline">Pendente</Badge></div>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader>
                    ...
                    <CardContent className='space-y-2'>
                        <Button variant="ghost" className='w-full justify-start p-2 h-auto text-left'><Syringe className="mr-2 h-4 w-4 shrink-0" /> Cartão de Vacinas <ChevronRight className='ml-auto h-4 w-4'/></Button>
                        <Button variant="ghost" className='w-full justify-start p-2 h-auto text-left'><AlertCircle className="mr-2 h-4 w-4 shrink-0" /> Acidentes de Trabalho <ChevronRight className='ml-auto h-4 w-4'/></Button>
                        <Button variant="ghost" className='w-full justify-start p-2 h-auto text-left'><Briefcase className="mr-2 h-4 w-4 shrink-0" /> Afastamentos INSS <ChevronRight className='ml-auto h-4 w-4'/></Button>
                    </CardContent>
                </Card>
            </aside>
          </div>
